...
import { Lock, Play, Square, Download, AlertCircle, Upload } from 'lucide-react';

const OpenRedirect: React.FC = () => {
  ...
  const handleRun = async () => {
    if (!uploadedFile) {
      alert('Please upload a URL list file first');
      return;
    }
    
    setIsRunning(true);
    setScanId(`openredirect_${Date.now()}`);
    setOutput('Starting open redirect detection...\n');

    try {
      const formData = new FormData();
      formData.append('file', uploadedFile);
      formData.append('tool', 'openredirex');
      formData.append('command', 'python3 openredirex.py -l urls.txt');
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `open_redirect_${Date.now()}.txt`;
    a.click();
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center space-x-3 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">Open Redirect Detection</h1>
          <p className="text-gray-400">Detect open redirect vulnerabilities using OpenRedireX</p>
        </div>
      </div>
      ...
      <div className="bg-pink-500/10 border border-pink-500/20 rounded-lg p-4 mb-6">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="text-pink-400 font-medium mb-1">Command Info</h4>
            <p className="text-sm text-gray-300 mb-2">
              Running: <code className="bg-dark-700 px-2 py-1 rounded text-primary font-mono">
                python3 openredirex.py -l {uploadedFile?.name || 'urls.txt'}
              </code>
            </p>
            <p className="text-xs text-gray-400">
              Upload a text file containing URLs with parameters (e.g., https://example.com/redirect?next=)
            </p>
          </div>
        </div>
      </div>
...
https://example.com/goto?next=
https://example.com/forward?redirect=
https://example.com/jump?to=
            ...
            <pre className="text-green-400 font-mono text-sm whitespace-pre-wrap">
              {output || (
                <span className="text-gray-500">
                  Upload a URL list file to begin scanning...
                  {isRunning && (
