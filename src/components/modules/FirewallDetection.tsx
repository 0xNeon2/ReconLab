...
import { Eye, Play, Square, Download, AlertCircle, Save } from 'lucide-react';

const FirewallDetection: React.FC = () => {
  ...
  const handleRun = async () => {
    ...
    setIsRunning(true);
    setScanId(`firewall_${Date.now()}`);
    setOutput('Starting firewall detection...\n');
  ...
  const handleDownload = () => {
    ...
    a.href = downloadUrl;
    a.download = `firewall_detection_${url.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}.txt`;
    a.click();
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex items-center space-x-3 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold text-white">Firewall Detection</h1>
          <p className="text-gray-400">Detect web application firewalls using wafw00f</p>
        </div>
      </div>
      ...
      <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 mb-6">
        <div className="flex items-start space-x-3">
          ...
          <div>
            <h4 className="text-yellow-400 font-medium mb-1">Command Info</h4>
